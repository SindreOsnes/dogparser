...
class UtStStat:
    ...
    _antavkutstbarpros: Union[int, None] # Percentage of children displayable (ANTAVKUTSTBARPROS in original schema)
    _antkullutstbar: Union[int, None] # Number of litters displayable (ANTKULLUTSTBAR in original schema)
    _antutstilt: Union[int, None] # Number of children displayed (ANTUTSTILT in original schema)
    _antutstiltpros: Union[int, None] # Percentage of children displayed (ANTUTSTILTPROS in original schema)
    ...
    def from_bytes(cls, content: bytes):
        if len(content) != 78:
            raise AssertionError("Input for UtStStat should be 78 bytes")

        # The Display Statistics table contains a 6 byte header that does not contain useful data
        ...
        antavkutstbarpros = struct.unpack('H', sub_content[:2])[0] # ANTAVKUTSTBARPROS (Percentage of children displayable is a 2 byte integer)
        ...
        antutstilt = struct.unpack('H', sub_content[:2])[0] # ANTUTSTILT (Number of children displayed is a 2 byte integer)
        ...
        antutstiltpros = struct.unpack('H', sub_content[:2])[0] # ANTUTSTILTPROS (Percentage of children displayed is a 2 byte integer)
...
class UtStStatList:
    ...
    _contents: List[UtStStat]

    def __init__(self, contents:List[UtStStat]=[]) -> None:
